...
# Broad Goals
The project's broad goal of is to explore the IMDB 5000 Movies dataset and address exciting issues such as the genre of movies that are mainly produced, IMDB score analysis, and profitability analysis. Moreover, if possible, I want to identify what set of features contribute to a highly rated/profitable film most significantly and try to predict a movie's profitability. 

Below are the R packages required for this project.
...
The dataset used in this project came from the IMDB 5000 Movie Dataset from Kaggle. It recorded information on 5043 movies across 66 countries from 1916 to 2016. The dataset is available in a csv format file and is of size 1MB. Note that the original dataset is replaced on Kaggle website, and therefore I cannot access the original one. The following link is where I sourced the same data from: https://www.kaggle.com/carolzhangdc/imdb-5000-movie-dataset [@Kaggle-IMDB-5000]. 

Each record in this dataset has 28 variables, including information such as `Title of the movie`, `Name of the movie director`, `Country the movie was produced in`, `Budget of the movie (in USD)`, `Profitability`, and `IMDB score for the movie`.
...
## Load Data
Import the data and show the dimension and names of all attributes of the data. There are 5043 movies recorded in this dataset, and each of which has 28 attributes. 
```{r laod-data}
...
In order to get the data ready for analysis, some tidying and cleansing works need to be done. First, unnecessary characters from the `movie_title`, `genre` and `plot_keyword` columns were removed.
```{r spur-char-rmv}
IMDB <- IMDB %>%
...
Second, duplicates in the data in `movie_title` column were removed as they may affect later analysis. In total, there are 126 duplicate movies being removed.
```{r duplicate-rmv}
sum(duplicated(IMDB$movie_title))
IMDB <- IMDB[!duplicated(IMDB$movie_title),]
```

Third, columns that contain currency, such as the `budget` and `gross`, may cause problems in later analysis because a few countries were not converted to US dollars, including "South Korea", "Japan", "Thailand", ..., etc. Furthermore, taking inflation into consideration makes the problem even more complicated. Thus, only movies from USA were kept for profitability analysis.
```{r currency-issue}
IMDB %>% select(movie_title, budget, country, gross) %>% arrange(desc(budget))
```

Then, an new column `profitable` was added to indicate if a movie is profitable, '1' means profitable (if `profit` $>$ `budget`). As this involving both `gross` and `budget` columns, only movies from USA have non-empty value for this column. Then, save it to an Excel file for people who want to focus only on USA films.
```{r imdb-usa}
IMDB.USA <- IMDB %>%
  filter(country == "USA") %>%
...
The exploratory analysis is done on four main aspects: Genre, Country, IMDB score, and Profitability. But before that, we can start with some simple yet interesting statistics about the data like "Top 10 something".
...
It follows that, when taking a closer look at relationship of these films with their IMDB ratings, there is little correlation found between them. This is as expected since 
