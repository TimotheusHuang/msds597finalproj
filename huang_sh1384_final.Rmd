...
Then, I created a column "profit_flag" to indicate if a movie is profitable, i.e, Revenue $>$ Budget, where 1 means profitable. As this involving both "gross" and "budget" columns, only movies from USA have non-empty value for this column. Then, save it to an Excel file for people who want to focus only on USA films.
```{r imdb-usa}
IMDB.USA <- IMDB %>%
  filter(country == "USA") %>%
...
Here is the preview of the cleaned data and it is saved to an Excel file.
```{r data-preivew}
...
Note that, after finishing all the cleaning process, there are 3768 rows that do not have any missing value and they are also saved to an Excel sheets for future use.
```{r complete-cases}
...
Var.tab <- tibble(
  ...
  Description = c(
    ...
    "Categorization: `Action`, `Comedy`, `Drama`, ..., etc.",
    "Name of Actor No.1",
    "Title of the Movie",
